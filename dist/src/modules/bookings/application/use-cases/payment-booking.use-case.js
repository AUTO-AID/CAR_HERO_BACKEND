    ...
    async pay(userId, bookingId, method) {
        ...
        if (updated.total > 0 && updated.user) {
            await this.walletRepository.executeTransaction('platform_earnings', 'system', async (platformWallet, session) => {
                ...
                const transaction = new transaction_entity_1.Transaction(transaction_entity_1.Transaction.generateTransactionNumber(), platformWallet.id, platformWallet.ownerId, platformWallet.ownerType, status_enum_1.TransactionType.CREDIT, updated.total, balanceBefore, balanceAfter, `Payment for booking #${updated.bookingNumber}`, undefined, 'booking', updated.id, method, undefined, 'completed');
