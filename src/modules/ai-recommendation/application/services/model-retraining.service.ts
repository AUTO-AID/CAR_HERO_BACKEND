    ...
    return new Promise((resolve) => {
      ...
      pyProcess.on('close', async (code) => {
        ...
        try {
          ...
          const reloadMessage = reloadErr.message || reloadErr.code || 'AI inference service is unavailable';
          this.logger.error(`FastAPI reload request failed: ${reloadMessage}`);
          resolve({
            success: true,
            logCount,
            fallbackUsed,
            message: `Model retrained successfully, but reload request failed: ${reloadMessage}`,
            output: stdoutData,
          });
        }
      });
