...
describe('SearchVehiclesUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<SearchVehiclesUseCase>(SearchVehiclesUseCase);
  ...
  describe('execute', () => {
    it('should throw BadRequestException for short query', async () => {
      await expect(useCase.execute('user1', 'a', 1, 10)).rejects.toThrow(
        BadRequestException,
      );
    });

    it('should throw BadRequestException for empty query', async () => {
      await expect(useCase.execute('user1', '', 1, 10)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should return cached results if available', async () => {
      ...
      const result = await useCase.execute('user1', 'Toyota', 1, 10);

      expect(cacheManager.get).toHaveBeenCalled();
      expect(vehicleRepository.search).not.toHaveBeenCalled();
      expect(result).toEqual(cachedResult);
    });

    it('should search vehicles and cache results', async () => {
      ...
      expect(vehicleRepository.search).toHaveBeenCalledWith('Toyota', 'user1', 0, 10);
